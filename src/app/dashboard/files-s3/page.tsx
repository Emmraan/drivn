...
export default function S3FilesPage() {
  ...
  const handleUploadComplete = useCallback(() => {
    logger.info("📤✅ Upload completed, refreshing UI...");
    refresh();
    setShowUploadModal(false);
  }, [refresh]);

  const handleFolderCreated = useCallback(() => {
    logger.info("📁✅ Folder created, refreshing UI...");
    refresh();
  ...
  const handleRenameSubmit = useCallback(
    async (newName: string) => {
      ...
      logger.info("✏️ Starting rename operation:", {
      ...
      try {
        ...
        if (renameItem.type === "file") {
          ...
          logger.error("❌ Unknown rename type:", renameItem.type);
          return;
        }

        if (result.success) {
          logger.info("✅ Rename operation successful");
          setShowRenameModal(false);
          setRenameItem(null);
        } else {
          logger.error("❌ Rename operation failed:", result.message);
          // You could show an error toast here
          alert(`Rename failed: ${result.message}`);
        }
      } catch (error) {
        logger.error("❌ Rename operation error:", error);
        alert(
          `Rename failed: ${
      ...
      {isLoading &&
        ...
        displayFolders.length === 0 && (
          <div
            className={
              viewMode === "grid"
                ? "mt-8 grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-6 gap-4"
                : "space-y-2"
            }
          >
      ...
      {(displayItems.length > 0 || displayFolders.length > 0) && (
        ...
        >
          ...
          {displayFolders.map((folder) => (
            <ContextMenu
              key={folder.key}
              items={getFolderContextMenuItems(folder)}
              itemType="folder"
            >
              <motion.div
              ...
              >
                ...
                >
                  ...
                  <button
                    ...
                    className={`${
                      viewMode === "grid" ? "absolute top-2 right-2" : "ml-2"
